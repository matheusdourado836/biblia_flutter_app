...
class _SavedVersesState extends State<SavedVerses> {
  ...
  void initState() {
    ...
    service.getAllBooks().then((value) => setState(() {futureList = value;}));
      ...
      appBar: AppBar(
        ...
        title: DropdownButton(
          ...
          onChanged: (newValue) {
            ...
            _savedVersesProvider.orderListByColor(_selectedOption.toLowerCase());
          },
        ),
      ),
      body: Container(
        ...
        child: Consumer<SavedVersesProvider>(
          builder: (context, list, child) {
            ...
            return coresListWidget(list: _savedVersesProvider.lista, corSelecionada: _selectedOption);
          },
        )
      ),
    );
  ...
  Widget coresListWidget({required List<VerseModel> list, required String corSelecionada}) {
    final List<dynamic> objetosFiltrados = corSelecionada.toLowerCase() == 'todas'
        ? list
        : list.where((objeto) => ConvertColors().convertColorsToText(objeto.verseColor).contains(corSelecionada.toLowerCase())).toList();
            ...
            onTap: (() {
              for(var element in futureList) {
                if(element.name == book) {
                  GoToVerseScreen().goToVersePage(book, element.abbrev, element.chapters, chapter, verseNumber);
                }
              }
            }),
            child: Card(
                ...
                children: [
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text('$book $chapter:$verseNumber', style: Theme.of(context).textTheme.titleLarge,),
